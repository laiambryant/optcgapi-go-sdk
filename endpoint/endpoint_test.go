...
func TestFetchSuccessAndDecodeErrorAndRequestError(t *testing.T) {
	...
	e2 := New[Item](c2)
	_, err = e2.Fetch(context.Background(), "/allSets/")
	if err == nil {
	...
	if !strings.Contains(derr.Error(), "/allSets/") {
	...
	e3 := New[Item](c3)
	_, err = e3.Fetch(context.Background(), "/allSets/")
	if err == nil {
